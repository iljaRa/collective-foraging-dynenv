...
void CFootBotForaging::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
      ...
      CRange<CDegrees> cGoStraightAngleRangeDegreesWithFood(CDegrees(-10.0f), CDegrees(10.0f));
      GetNodeAttribute(t_node, "go_straight_angle_range", cGoStraightAngleRangeDegrees);
      GetNodeAttribute(t_node, "go_straight_angle_range_with_food", cGoStraightAngleRangeDegreesWithFood);
      GoStraightAngleRange.Set(ToRadians(cGoStraightAngleRangeDegrees.GetMin()),
                               ToRadians(cGoStraightAngleRangeDegrees.GetMax()));
      GoStraightAngleRangeWithFood.Set(ToRadians(cGoStraightAngleRangeDegreesWithFood.GetMin()),
                                       ToRadians(cGoStraightAngleRangeDegreesWithFood.GetMax()));
...
CFootBotForaging::SStateData::SStateData() :
   ProbRange(0.0f, 1.0f),
   InitialRestToExploreProb(0.01),
   InitialExploreToRestProb(0.01),
   packetDropProb(0.0) {}

void CFootBotForaging::SStateData::Init(TConfigurationNode& t_node) {
   try {
      GetNodeAttribute(t_node, "individual_rule_explore_to_rest_delta_prob", IndivRuleExploreToRestDeltaProb);
      GetNodeAttribute(t_node, "individual_rule_rest_to_explore_delta_prob", IndivRuleRestToExploreDeltaProb);
      ...
      GetNodeAttribute(t_node, "packet_drop_prob", packetDropProb);
...
void CFootBotForaging::Init(TConfigurationNode& t_node) {
   try {
      ...
      isScaleFreeCommunication = false;
   }
   catch(CARGoSException& ex) {
...
void CFootBotForaging::ControlStep() {
   ...
   /* Evaluating the current state of the foot-bot */
   switch(m_sStateData.State) {
      ...
      default: {
         LOGERR << "We can't be here, there's a bug inside the ControlStep() function!" << std::endl;
      }
   }
}
...
double CFootBotForaging::GetMaxSpeed(){
...
void CFootBotForaging::Reset() {
   ...
   m_unFBMsg = 0; // The Id integer number received by the sensor
   strID = m_strId.substr (2,5); // The Id string
   m_unID = (UInt64) std::stoi(strID,nullptr,10); // The Id integer number that is sent by the actuator, converted from strID
   m_unHasFoodItem = 1;
   b_leftNest = false;
   time = 0;
   
   /*
    * Careful: m_pcRABA->SetData(size_t,UInt8)
    * This means, to send m_unID (which can be > 255)
    * compatible with UInt8, the m_unID is decomposed into the
    * number of times 255 is inside, plus rest.
    * When the message is read inside the loop_functions, it is read like this:
    * m_unFBMsg = tPacketsLOG[i].Data[1]*255 + tPacketsLOG[i].Data[2];
   */
   UInt8 rest = m_unID % 255;
   UInt8 multiplier = (m_unID - rest) / 255;
   m_pcRABA->SetData(1,multiplier);
   m_pcRABA->SetData(2,rest);
...
bool CFootBotForaging::IsCollAvoiding() {
   /* Get readings from proximity sensor */
   const CCI_FootBotProximitySensor::TReadings& tProxReads = m_pcProximity->GetReadings();
   /* Sum them together */
   CVector2 cDiffusionVector;
...
void CFootBotForaging::SetWheelSpeedsFromVector(const CVector2& c_heading) {
   ...
   /* fdampingFactor is useful because otherwise footbots turn too quickly.
    * Consequently, they tend to get stuck at walls because they turn so fast that the rear sensors
    * sense the wall (so again a large angle for collision avoidance).
    * Subsequently, the footbot performs another hard turn, senses the wall
    * and perfomrs again a hard turn, etc. etc.
   */
    Real fdampingFactor = (m_sWheelTurningParams.NoTurnAngleThreshold * 1.0 + Abs(cHeadingAngle) * 1.0) / (Abs(cHeadingAngle) * 2.0);
   ...
   switch(m_sWheelTurningParams.TurningMechanism) {
      case SWheelTurningParams::NO_TURN: {
         /* Just go straight */
         /* Idealy no_turn_angle_threshold="10" */
      ...
      case SWheelTurningParams::SOFT_TURN: {
         ...
         /* Idealy soft_turn_angle_threshold="30".
          * If higher, the damping factor should be adjusted
          * such that the damping is stronger
         */

         fBaseAngularWheelSpeed *= fdampingFactor;
      ...
      case SWheelTurningParams::HARD_TURN: {
         ...
         /* Idealy hard_turn_angle_threshold="90"! */

         fSpeed1 = -m_sWheelTurningParams.MaxSpeed * fdampingFactor;
         fSpeed2 =  m_sWheelTurningParams.MaxSpeed * fdampingFactor;
         break;
      }
   }
...
void CFootBotForaging::ListenToNeighbs(const std::vector<UInt8>& newNeighbInfo){
   /* Create a deep copy of the information passed from the loop functions */
   /* newNeighbInfo contains information sent over the links of a synthetically generated scale-free network. */
   if(m_sStateData.State == SStateData::STATE_RESTING){
	  for(int i = 0; i < newNeighbInfo.size(); i++) {
		 /* Drop packet with probability m_sStateData.packetDropProb */
		 if(m_sStateData.packetDropProb > 0.0){ 
			 if(m_pcRNG->Uniform(m_sStateData.ProbRange) < m_sStateData.packetDropProb){ 
				  continue; 
			 } 
		 }
		 /* Read the neighbor i's last exploration attempt. */
		 if(newNeighbInfo[i] == LAST_EXPLORATION_SUCCESSFUL) {
		 ...
		 else if(newNeighbInfo[i] == LAST_EXPLORATION_UNSUCCESSFUL){
			   m_sStateData.ExploreToRestProb += m_sStateData.SocialRuleExploreToRestDeltaProb;
			   m_sStateData.ProbRange.TruncValue(m_sStateData.ExploreToRestProb);
			   m_sStateData.RestToExploreProb -= m_sStateData.SocialRuleRestToExploreDeltaProb;
			   m_sStateData.ProbRange.TruncValue(m_sStateData.RestToExploreProb);
		}
		 else if(newNeighbInfo[i] == LAST_EXPLORATION_NONE){}
		 else{
			LOGERR << "We can't be here, there's a bug in handling communication inside the ListenToNeighbs() function!" << std::endl;
		 }
	  }
  }
}
...
void CFootBotForaging::SetScaleFreeCommunication(bool isSF){
	isScaleFreeCommunication = isSF;
}
...
void CFootBotForaging::Explore() {
   ...
   if(m_sFoodData.HasFoodItem) {
       ...
       * RestToExploreProb */
      m_sStateData.ExploreToRestProb -= m_sStateData.IndivRuleExploreToRestDeltaProb;
      m_sStateData.ProbRange.TruncValue(m_sStateData.ExploreToRestProb);
      m_sStateData.RestToExploreProb += m_sStateData.IndivRuleRestToExploreDeltaProb;
      m_sStateData.ProbRange.TruncValue(m_sStateData.RestToExploreProb);
...
void CFootBotForaging::ReturnToNest() {
   ...
   if(m_sStateData.InNest) {
	 ...
	 /* Save the last exploration attempt in the actuator if we managed to leave the nest */
	 if(b_leftNest) {
