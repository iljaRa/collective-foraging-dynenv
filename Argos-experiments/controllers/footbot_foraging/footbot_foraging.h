...
public:
   ...
   struct SDiffusionParams {
      ...
      CRange<CRadians> GoStraightAngleRangeWithFood;
   ...
   struct SStateData {
      ...
      Real IndivRuleExploreToRestDeltaProb;
      /* The increase of RestToExploreProb due to the food rule */
      Real IndivRuleRestToExploreDeltaProb;
      ...
      /* The probability of a random packet drop */
	  Real packetDropProb;
...
public:
   ...
   /*
    * Returns true if the robot is currently inside the nest.
    */
   inline bool IsInNest() const {
      return m_sStateData.InNest;
   }
    ...
    * Returns the food data
    */
   inline UInt8 GetLastExplResults() {
      return m_eLastExplorationResult;
   }

   /*
    * Returns true if collision avoidance happened
    */
    bool IsCollAvoiding();

    /*
     * Returns the maximum speed of the footbot.
     */
    double GetMaxSpeed();

    void ListenToNeighbs(const std::vector<UInt8>& newNeighbInfo);
    
    void SetScaleFreeCommunication(bool isSF);
...
private:
   ...
   CVector2 DiffusionVectorWithFood(bool& b_collision);
   ...
   /* Pointer to the range and bearing sensor for sensing theId of the neighboring robots */
   CCI_RangeAndBearingSensor* m_pcRABSforId;
   ...
   /* Counter whose value is sent to the neighboring foot-bots */
   UInt64 m_unID;
   std::string strID;

   /* Count the number of found food-items */
   UInt64 m_unHasFoodItem;

   /* True if the state of a robot inside the nest has been changed to 'Explore' */
   bool b_changedState;

   /* True when there is a nest in the arena */
   bool b_leftNest;
